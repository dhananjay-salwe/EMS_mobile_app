...
import {
  ...
  Image, TouchableOpacity, Alert, KeyboardAvoidingView, Platform, ActivityIndicator, StyleSheet,
} from 'react-native';
...
export default function VoteSubmissionScreen({ operator, selectedBooth }) {
  ...
  const fetchCandidates = async () => {
    try {
      const data = await apiCall(`/candidates/by-booth/${selectedBooth.booth_id}`);
      if (data.success) {
  ...
  const updateVote = (candidateId, text) => {
    ...
    setVotes(prev => ({ ...prev, [candidateId]: text.replace(/[^0-9]/g, '') }));
  };

  const pickImage = async () => {
    const { status } = await ImagePicker.requestCameraPermissionsAsync();
    if (status !== 'granted') {
      Alert.alert('Permission Denied', 'Camera access is required to capture the tally sheet.');
      return;
    }
  ...
  const submitData = async () => {
    ...
    for (const c of candidates) {
      ...
      if (isNaN(count)) {
        Alert.alert('Validation Error', `Please enter valid vote counts for ${c.candidate_name}.`);
        return;
      }
    ...
    if (!tallyImage) {
      Alert.alert('Missing Photo', 'Please capture the physical tally sheet photo before submitting.');
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        Alert.alert('Success!', 'Vote results and tally sheet photo successfully recorded.');

        // Reset form for potential re-submission (or they can just logout)
        const resetVotes = {};
        candidates.forEach(c => { resetVotes[c.candidate_id] = ''; });
        setVotes(resetVotes);
        setTallyImage(null);
      } else {
  ...
  return (
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1, backgroundColor: COLORS.bodyBg }}>
      <FlatList
        data={candidates}
        keyExtractor={item => item.candidate_id.toString()}
        contentContainerStyle={{ padding: SPACING.md }}
        ListHeaderComponent={
          <Text style={styles.sectionLabel}>Enter votes counted for each candidate</Text>
        }
        renderItem={({ item }) => (
          <View style={styles.candidateRow}>
            {item.party_icon_url ? (
              <Image source={{ uri: item.party_icon_url }} style={styles.partyIcon} />
            ) : (
              <View style={styles.partyIconPlaceholder}>
                <Text style={styles.partyIconInitial}>{item.party_name?.charAt(0)}</Text>
              </View>
            )}
            <View style={styles.candidateInfo}>
              <Text style={styles.candidateName}>{item.candidate_name}</Text>
              <Text style={styles.partyName}>{item.party_name} ({item.party_code})</Text>
            </View>
            <TextInput
              ...
              value={votes[item.candidate_id] ? votes[item.candidate_id].toString() : ''}
        ...
        ListFooterComponent={
          <View style={styles.footer}>
            <Button
              title={tallyImage ? '📸  Retake tally sheet photo' : '📸  Capture tally sheet photo'}
              variant="dark"
              onPress={pickImage}
            ...
            {tallyImage && (
              <Image source={{ uri: tallyImage.uri }} style={styles.previewImage} />
            )}

            <Button
              title={submitting ? 'Submitting to server…' : 'Submit Final Votes'}
              variant="primary"
              onPress={submitData}
              loading={submitting}
            />
...
const styles = StyleSheet.create({
  ...
  partyIcon: { width: 40, height: 40, borderRadius: RADIUS.pill, marginRight: 12 },
  partyIconPlaceholder: {
    width: 40, height: 40, borderRadius: RADIUS.pill, marginRight: 12,
    backgroundColor: COLORS.primarySoft, alignItems: 'center', justifyContent: 'center',
  },
  partyIconInitial: { color: COLORS.primary, fontWeight: '700', fontSize: 15 },
  candidateInfo: { flex: 1 },
  candidateName: { fontSize: 15, fontWeight: '700', fontFamily: FONTS.bold, color: COLORS.textDark },
